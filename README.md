...
- 構成: **Core(AutoCAD 非依存の計算層)/ Plugin(AutoCAD 依存層)** の 2 プロジェクト分割。Core は BCL のみを参照し、WSL / Linux でもテストできる(**xUnit 585 ケース green**)
...
### `SpqwNodes.CalcWeightedN`

柱状図 CSV(§5.6)から加重平均N値・加重平均一軸圧縮強度を算出する。AutoCAD のトランザクション・ジオメトリを伴わない純計算のため、Dynamo の `File Path` ノード等からファイルパスを直接配線できる。行に 1 件でも不備があれば例外を投げて計算全体を止める(既存の帳票 CSV 取り込みコマンドとは異なり、部分的な値のまま地盤条件の計算を進めると設計判断を誤るため、あえて全件成功を必須にしている)。

```
 [String] csvPath ── SpqwNodes.CalcWeightedN ──┬── 加重平均N値 (R用、N=0連続除外)
                                               ├── 根入れ長 (R用) [m]
                                               ├── 加重平均N値 (Sb用、N≦5連続除外)
                                               ├── 根入れ長 (Sb用) [m]
                                               ├── 加重平均N値 (砂質土等)
...
| 入力 | 単位 | デフォルト値 |
|---|---|---|
| `csvPath` | − | `""`(未指定・ファイル不存在・行の不備はいずれも例外) |

出力される加重平均N値・N値・qu が該当層無しで算出不能な場合(例:柱状図に岩盤層が無い)は空文字を返す。
...
```bash
# Core + テスト(AutoCAD 不要。585 件が green であること)
...
│       ├── Dynamo/SpqwNodes.cs          Zero Touch Node 3 個(柱状図解析は AutoCAD 非依存の
│       │                                 純計算のためファイルパスを直接受け取る)
│       └── DrawingHelper.cs / Prompt.cs / SolidBuilder.cs
├── stubs/                               AutoCAD API スタブ(構文検証専用。配布禁止)
├── tests/SheetPileQuayWall.Core.Tests/  xUnit 585 ケース + fixtures/
├── scripts/
...
- **§4・§5.6・§6.8 柱状図から加重平均N値を算出する Dynamo ノード `SpqwNodes.CalcWeightedN` を追加**。前壁・バイブロ単独・控え杭・ジェット併用の各打設歩掛コマンドが個別に尋ねていた `nAvg` 等を、標高上端・標高下端・層厚・N値・土質区分(5区分)・打止め換算(打撃回数法・貫入量)・岩盤の一軸圧縮強度を持つ柱状図 CSV から算出できるようにした。R 用(N=0除外)と Sb 用(N≦5除外)で異なる加重平均N値になることを踏まえ、両方を別々に算出する。ジオメトリ・AutoCAD トランザクションを伴わない純計算のため、AutoCAD コマンドではなく Dynamo ノードとして実装し、ファイルパスを直接受け取る。新規 `Core.Geotech` 名前空間を新設し、土質区分は既存の `FrontWall.JetLayerType` をそのまま流用した。
- **§6.3・§6.4 付帯船舶(台船・引船・揚錨船・潜水士船)を追加**。従来 `SPQW_FRONTWALL_VibroEstimate` は起重機船・杭打船のトン数までしか選定しておらず、付帯船舶は固定文字列の注記に留まっていた。基準原文(4節 3-4.6-6「２－３－２－２ 作業船・機械の組合せ」)を確認し、台船・引船を積載物の長さ(杭の全長)から選定する `VibroEstimate.GetBargeAndTug` を新設。揚錨船・潜水士船は規格が固定であることを確認し定数化した。潜水士船は既存の `obstacle`(障害区分)とは別の判断軸(打設個所の障害物・打設後異常の調査作業の要否)であるため新規プロンプトを追加。`SPQW_FRONTWALL_VibroJetEstimate` にも海上打設時のみ同じ規格表(3-16-18 注2 で共有が確認済み)を適用した。§9.2 にメイン船のトン数ランク選定が未実装である旨を追記。
- **§3 控え杭の打撃工法・積算(陸上打設)**: `SPQW_ANCHORPILE_Estimate` を新設。控え杭は継手を持たない単独の鋼管杭であるため、前壁(4節 3-4.5「鋼矢板式」)ではなく **4節 3-4.6「鋼杭式」**を出典とする。原文突き合わせの結果、貫入抵抗値 R・ハンマ規格・打撃速度 Sb 表・溶接時間表・準備時間 Tp・基準作業能力係数は両節で数値まで一致するため共有実装を再利用し、唯一の実質差(打撃時間 Tb の係数 K、直杭1.0/斜杭1.2)のみ新規実装した。副産物として、既存 `FrontWall.DriveEstimate.GetLabor`(前壁)の労務編成に杭長のしきい値バグを発見し §9.3 に記録した(修正は別件)。
...
- 数値の更新: テスト 382 → 475 → 524 → 542 → 556 → **585 ケース**、コマンド 13 → 14 → 18 → **19 個**(付帯船舶・柱状図解析はいずれも既存コマンド/新規 Dynamo ノードへの機能追加でありコマンド数は変わらない)、Dynamo ノード 2 → **3 個**。
