...
- 前壁の矢板ピッチ(円中心間距離)は既にカタログ有効幅 B を使っていたが、継手の実 3D 形状(`JointShapes`、007 の DXF 抽出データ)と B の算出式(`JointGeometry`、カタログの解析式)は別々のデータソースであり、B ピッチで配置した際に継手同士が視覚的に破綻しないかは未検証だった。
- 単純な「A側・B側それぞれの最大/最小 Y 座標を比較する」方法では、比較する端点を誤ると結論が逆転する(隙間に見えた数値が、比較し直すと大きなめり込みに見えるなど)ことが判明したため、2D 多角形同士の正式な交差判定(頂点内包 + 辺の交差判定、`JointFit.Overlaps`)に置き換えて再検証した。
- 結果、**LT65/LT75/LT100 は D=500〜2000mm の範囲で干渉せず、最小離隔は径によらず約5.5mmで一定**であることを確認した(`JointPlacement` の変換式が A側・B側とも半径 R を単純加算する構造のため、相対位置関係は R に依存せず継手有効間隔 J だけで決まる)。**継手 3D モデル側の補正は不要**と判断し、`BuildSolid` 等の生成コードは変更していない。
- **PP/PT(円形継手管 φ165.2 が絡み合う構造)は対象外**。LT型のような「差し込み‐受け」構造ではないため、多角形交差判定では噛み合わせの実態を正しく評価できない。別途 3D 的な評価が必要(未着手)。
- `JointFitTests`(T1280〜T1286)で、この整合性を回帰テストとして固定した。今後 `JointShapes`(DXF再抽出)や `JointGeometry`(カタログ式)側が変更された際に、整合性が崩れれば検出できる。
