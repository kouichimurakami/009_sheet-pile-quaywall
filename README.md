...
- 構成: **Core(AutoCAD 非依存の計算層)/ Plugin(AutoCAD 依存層)** の 2 プロジェクト分割。Core は BCL のみを参照し、WSL / Linux でもテストできる(**xUnit 475 ケース green**)
...
全 **14 コマンド**。接頭辞は `SPQW`(Sheet Pile Quay Wall)。命名は次のパターンに従う。
...
```bash
# Core + テスト(AutoCAD 不要。475 件が green であること)
...
├── tests/SheetPileQuayWall.Core.Tests/  xUnit 475 ケース + fixtures/
├── scripts/
...
| 4 | **前壁の壁一括生成が無い**。矢板 1 本ごとに平面位置をピックする方式(006 / 007 から踏襲)のため、100 本の壁では 100 回の操作になる。ピッチ B での自動配置は未実装。 |
...
- **§9 注意点・既知の課題**: 全面的に再構成。旧版は箇条書き 8 項目だったものを、**9.1 積算基準データの復元限界(要・原本確認)/ 9.2 実装範囲の限定 / 9.3 モジュール間で挙動が揃っていない点 / 9.4 移植元リポジトリの不整合 / 9.5 実機動作確認** の 5 分類に整理し、表形式で影響と対処を明示した。冒頭にも §9 を読むよう誘導を追加。
- 数値の更新: テスト 382 → **475 ケース**、コマンド 13 → **14 個**。
