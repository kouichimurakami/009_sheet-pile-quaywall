...
3. ~~**Dynamo ノードの対象範囲**~~ → **解決済み(第3版(g))**: 前壁の計算ノードのみを実装する。`SpqwNodes.CalcSection`(007 由来)と `SpqwNodes.CalcQuayWallQuantity`(施設数量、009 新規)の 2 個。ジオメトリ生成ノード(007 `CreateSolid` 相当)は AutoCAD のトランザクションを伴い実機でしか検証できないため移植しない。`ProtoGeometry` は参照せず `DynamoServices` のみ(`ExcludeDynamo=true` で除外可)。
