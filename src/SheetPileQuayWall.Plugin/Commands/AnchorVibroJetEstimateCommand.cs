...
//
// 控え杭のジェット併用バイブロハンマ打設積算
// (SPQW_ANCHORPILE_VibroJetEstimate、陸上/海上とも)
// 出典: 港湾土木請負工事積算基準 令和7年度改訂版 3章16節 3-1(3-16-11〜25)
//
// 追加の根拠(2026-08-01): 4節 3-4.6-9 の適用工法表は振動工法(バイブロハンマ)を
// 標準工法として挙げ、施工歩掛は「16節 仮設工」によれると注記する。16節 3-1 は
// 「ウォータージェットを併用したバイブロハンマによる**鋼管杭**・鋼管矢板の打設」で
// あり陸上/海上とも規定を持つため、**陸上の控え杭に振動工法を適用できる唯一の
// 基準準拠経路**である(バイブロ単独の 16節 3-2 は海上限定)。
//
// 前壁版(VibroJetEstimateCommand)との違い:
//   - 杭 1 本当り質量 Wp は本管のみ(控え杭は継手を持たない単独杭)
//   - 継手加算時間 ε = 0(継手長 ℓj を尋ねない)
//   - 斜杭は基準上「別途検討」のため警告を出す(AnchorEstimateHelper)

namespace SheetPileQuayWall.Plugin.Commands
{
    public static class AnchorVibroJetEstimateCommand
    {
        // ════════════════════════════════════════════════════════════════════
        // SPQW_ANCHORPILE_VibroJetEstimate: 控え杭選択 → ジェット併用の施工歩掛積算
        // ════════════════════════════════════════════════════════════════════
        [Autodesk.AutoCAD.Runtime.CommandMethod("SPQW_ANCHORPILE_VibroJetEstimate")]
        public static void Estimate()
        {
            ...
            // ── 杭 1 本当り質量 Wp(本管のみ。控え杭は継手を持たない)────────
            double bodyMass_kg =
                SheetPileQuayWall.Core.FrontWall.SectionProperties.CalcW(
                    a.OuterDm, a.WallTm) * a.LengthM;
            ...
            double liftLength_m;
            if (!SheetPileQuayWall.Plugin.Prompt.TryAskDouble(
                ed, $"\n吊込 1 回ごとの杭長 L0 (m) <{a.LengthM:F1}>: ",
                a.LengthM, 1.0, 80.0, out liftLength_m))
            {
                return;
            }
            ...
            // 控え杭は継手を持たないため継手加算時間 ε = 0(前壁は ℓj を尋ねて 0.3×ℓj)
            ...
            // 溶接時間は 4節 3-4.5/4.6 の表による(3-16-21 の表と同一データ)
            ...
            ed.WriteMessage("\n--- 杭 1 本当り質量 Wp ---");
            ed.WriteMessage($"\n  本管        : {bodyMass_kg,10:F0} kg  [確定](K011 単位重量 × L)");
            ed.WriteMessage($"\n  合計 Wp     : {pileMass_t,10:F3} t  (継手なし。閉端底板は含めない)");
            ...
            ed.WriteMessage(
                site == SheetPileQuayWall.Core.FrontWall.ConstructionSite.Onshore
                ? "\n  (クローラクレーンの規格は上記吊上げ荷重から別途選定すること、3-16-18)"
            ...
            ed.WriteMessage($"\n  ε (継手加算): {epsilon,8:F2} 分  (控え杭は継手が無いため 0)");
            ed.WriteMessage($"\n  準備時間 Tp : {Tp,8:F1} 分/本  ((0.3·L0 + 11)×ns + 5)");
            ed.WriteMessage($"\n  打込時間 Tb : {Tb,8:F1} 分/本  (γ·β·δ·ℓ + ε)");
            ed.WriteMessage($"\n  溶接時間 Tw : {Tw,8:F1} 分/本  (4節 4.5/4.6 を適用)");
            ed.WriteMessage($"\n  打設時間 Tc : {Tc,8:F1} 分/本");
            ed.WriteMessage($"\n  日当り打設  : {Q,8:F2} 本/日  (ei=" +
            ...
            ed.WriteMessage("\nSPQW_ANCHORPILE_VibroJetEstimate 完了。");
        }
